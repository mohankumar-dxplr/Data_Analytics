"""
Build clean client executive report with proper table formatting.
No special Unicode characters. All $ signs preserved.
"""
import os

ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

content = """# Contoso Retail -- Client Executive Report
...
# No placeholder replacements needed - content is already clean

# Write root version
root_path = os.path.join(ROOT_DIR, 'CLIENT-EXECUTIVE-REPORT.md')
with open(root_path, 'w', encoding='utf-8') as f:
    f.write(content)
print(f"Root version written: {root_path} ({len(content)} bytes)")

# Write reports version (with ./figures/ paths)
rpt_path = os.path.join(ROOT_DIR, 'reports', 'CLIENT-EXECUTIVE-REPORT.md')
rpt_content = content.replace('reports/figures/', './figures/')
with open(rpt_path, 'w', encoding='utf-8') as f:
    f.write(rpt_content)
print(f"Reports version written: {rpt_path} ({len(rpt_content)} bytes)")

# Verification
import re
for path, label in [(root_path, 'Root'), (rpt_path, 'Reports')]:
    ...
    for i, line in enumerate(lines, 1):
        if line.startswith('|') and '$' in line:
            ...
            for ci, cell in enumerate(cells):
                if cell and cell[0].isdigit() and '$' not in cell and cell[0] not in '*':
                    dollar_issues.append(f"  L{i} col{ci}: '{cell}'")
    ...
    print(f"  Images: {len(re.findall(r'\.png\)', text))}")
    print(f"  Empty table cells: {len(empty_lines)}")
    if empty_lines:
        print(f"  Lines with empty cells: {empty_lines}")
    print(f"  Dollar-value issues: {len(dollar_issues)}")
    if dollar_issues:
        for di in dollar_issues[:10]:
            print(f"  {di}")
